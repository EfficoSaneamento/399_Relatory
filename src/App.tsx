...
type RecordItem = { id: number; address: string; city: string; nucleus: string; propertyType: string; inspectedAt: string; latitude: number | null; longitude: number | null; photos: string[]; details: Array<Record<string, unknown>> }
...
function mapLinkUrl(lat: number, lon: number) {
...
const demoRecords: RecordItem[] = [
  { id: 1, address: 'Rua Emiliano Di Cavalcanti, 543', city: 'Franca', nucleus: 'Franca 01', propertyType: 'Territorial', inspectedAt: '29 jun. 2026, 09:48', latitude: null, longitude: null, photos: [], details: [] },
  { id: 2, address: 'Rua Anita Malfati, 670', city: 'Franca', nucleus: 'Franca 01', propertyType: 'Predial', inspectedAt: '29 jun. 2026, 09:27', latitude: null, longitude: null, photos: [], details: [] },
  { id: 3, address: 'Rua José Vicente, 07', city: 'Franca', nucleus: 'Franca 02', propertyType: 'Predial', inspectedAt: '15 jul. 2026, 14:53', latitude: null, longitude: null, photos: [], details: [] },
]
...
function App() {
  ...
  async function loadRecords() {
    ...
    try {
      ...
      const data: { registros: Array<{ id: number; endereco: string; numero_imovel?: string; cidade: string; nucleo: string; tipo_imovel: string; data_inspecao: string; latitude?: number | null; longitude?: number | null; imagens: string[]; detalhes?: Array<Record<string, unknown>> }> } = await response.json()
      const mapped = data.registros.map((item) => ({ id: item.id, address: formatAddress(item.endereco, item.numero_imovel), city: item.cidade.trim(), nucleus: item.nucleo.trim(), propertyType: item.tipo_imovel, inspectedAt: item.data_inspecao, latitude: item.latitude ?? null, longitude: item.longitude ?? null, photos: item.imagens.map((image) => `${import.meta.env.BASE_URL}${image}`), details: item.detalhes ?? [] }))
  ...
  async function buildRecordSection(record: RecordItem) {
    ...
    const mapHtml = record.latitude !== null && record.longitude !== null
      ? `<div class="map"><iframe src="${mapEmbedUrl(record.latitude, record.longitude)}" loading="lazy"></iframe><p>${record.latitude.toFixed(6)}, ${record.longitude.toFixed(6)} · <a href="${mapLinkUrl(record.latitude, record.longitude)}" target="_blank" rel="noreferrer">Ver no mapa ↗</a></p></div>`
      : '<p>Coordenadas não disponíveis para esta vistoria.</p>'
    return `<section id="vistoria-${record.id}" class="record-report"><h2>${record.address}</h2><div class="meta"><p>${record.city} · ${record.nucleus}<br>Tipo: ${record.propertyType}<br>Inspeção: ${record.inspectedAt}</p></div><h3>Localização</h3>${mapHtml}<h3>Detalhamento da vistoria</h3><table>${detailHtml}</table><h3>Imagens da vistoria</h3><div class="photos">${photoHtml || '<p>Nenhuma imagem anexada a esta vistoria.</p>'}</div></section>`
  }
